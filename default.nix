...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    router.networkNamespaces = builtins.zipAttrsWith (k: vs: { }) (
    ...
    systemd.services =
      lib.flip lib.mapAttrs' cfg.interfaces (
        interface: icfg:
        let
        ...
        in
        {
          ...
          value =
            assert lib.assertMsg (!config.networking.interfaces ? ${interface})
      ...
      // lib.flip lib.mapAttrs' cfg.veths (
        interface: value:
        let
          escapedInterface = utils.escapeSystemdPath interface;
      ...
      // lib.flip lib.mapAttrs' cfg.tunnels (
        interface: value:
        let
          escapedInterface = utils.escapeSystemdPath interface;
          flags =
            [ interface ]
            ++ lib.optionals (value.mode != null) [
              "mode"
              value.mode
            ]
            ++ lib.optionals (value.remote != null) [
              "remote"
              value.remote
            ]
            ++ lib.optionals (value.local != null) [
              "local"
              value.local
            ]
            ++ lib.optionals (value.ttl != null) [
              "ttl"
              (toString value.ttl)
            ];
        in
      ...
      // lib.flip lib.mapAttrs' (lib.filterAttrs (k: v: v.rules != [ ]) cfg.networkNamespaces) (
        name: value:
        let
          args =
            x:
          ...
          value = {
            ...
            script = builtins.concatStringsSep "\n" (
              map (rule: ''
                ...
                mkdir -p $(dirname "$state")
                if out=$(ip ${if rule.ipv6 then "-6" else "-4"} rule add ${args rule} 2>&1 && echo ${
                  lib.escapeShellArg ("ip ${if rule.ipv6 then "-6" else "-4"} rule del " + args rule)
                } >> "$state"); then
                  echo "done"
                elif ! echo "$out" | grep "File exists" >/dev/null 2>&1; then
                  echo "'ip ${
                    if rule.ipv6 then "-6" else "-4"
                  } rule add "${lib.escapeShellArg (args rule)}"' failed: $out"
                  exit 1
                fi
              '') value.rules
            );
      ...
      //
        lib.flip lib.mapAttrs'
          (lib.filterAttrs (
            k: v: builtins.any (x: x != null) (builtins.attrValues v.sysctl)
          ) cfg.networkNamespaces)
          (
            name: value: {
              ...
              value = {
                ...
                serviceConfig.ExecStart = "${lib.getExe pkgs.sysctl} ${
                  lib.escapeShellArgs (
                    lib.mapAttrsToList (k: v: "${k}=${if v == false then "0" else toString v}") (
      ...
      // lib.flip lib.mapAttrs' cfg.networkNamespaces (
        name: value: {
          name = "netns-${name}";
          value =
            {
            ...
            // lib.optionalAttrs (name != "default" || value.extraStopCommands != "") {
              postStop =
                lib.optionalString (value.extraStopCommands != "") ''
      ...
      // lib.flip lib.mapAttrs' (lib.filterAttrs (k: v: router-lib.requiresNetnsSetup k) cfg.interfaces) (
        interface: value:
        let
          escapedInterface = utils.escapeSystemdPath interface;
            ...
            router-lib.mkServiceForIf'
              ...
              )
              {
                ...
                preStop =
                  if isVeth then
                    ''
                      ip netns exec "${value.networkNamespace}" ip link set "${interface}" netns "${
                        if peerNs != null then peerNs else "1"
                      }"
                    ''
                  else
                    ''
      ...
      // {
        network-setup = {
          partOf = map (name: "network-addresses-${utils.escapeSystemdPath name}.service") (
            ...
            map (service: {
              ${service.service or service} =
                if builtins.isString service then
                  ...
                  router-lib.mkServiceForIf' (
                    builtins.removeAttrs service [ "service" ] // { inherit interface; }
                  ) { };
            }) icfg.dependentServices
          ) cfg.interfaces
        )
      );
    systemd.network.links = lib.flip lib.mapAttrs' cfg.interfaces (
      name: value: {
        name = "40-${name}";
        value = {
          matchConfig =
            if value.systemdLink.matchConfig == { } then
              { OriginalName = name; }
            else
              value.systemdLink.matchConfig;
          linkConfig = value.systemdLink.linkConfig;
        };
      }
    );
    networking.useDHCP = lib.mkIf (builtins.any (x: x.dhcpcd.enable) (
