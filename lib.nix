...
in lib.optionalAttrs (config != null && utils != null) rec {
  ...
  mainDepForIf' = { interface, preNetns ? false, inNetns ? !preNetns, ... }: let
    ...
    in if !preNetns && netns != null && requiresNetnsSetup interface then
  ...
  mkServiceForIf' = args0@{ interface, includeBasicDeps ? true, preNetns ? false, inNetns ? !preNetns, bindType ? "bindsTo" }: attrs: let
    netns = cfg.interfaces.${interface}.networkNamespace or null;
    deps =
