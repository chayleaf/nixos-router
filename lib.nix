...
let
  cfg = config.router;
  vethParents = lib.mapAttrs' (name: value: {
    name = value.peerName;
    value = name;
  }) cfg.veths;
  bridges = builtins.zipAttrsWith (k: builtins.filter (x: x != null)) (
    builtins.filter (x: x != null) (
      lib.mapAttrsToList (
        interface: icfg:
        if icfg.bridge == null then
          null
        else
          {
  ...
  longestZeroSeq = lib.flip builtins.foldl' { } (
    {
      curLen ? 0,
      maxLen ? 0,
      curStart ? -1,
...
in
lib.optionalAttrs (config != null && utils != null) rec {
  ...
  requiresNetnsSetup =
    ...
    if vethParents ? ${interface} then
      (cfg.interfaces.${vethParents.${interface}}.networkNamespace or null)
      != (cfg.interfaces.${interface}.networkNamespace or null)
    else
...
// rec {
  ...
  # generate 0b11111...
  gen1Bits = count: if count == 0 then 0 else (gen1Bits (count - 1)) * 2 + 1;
  ...
  ip6Regex =
    let
      ...
      compStartExact = n: if n == 1 then "${compRegex}:" else "${compStartRegex}{${toString n}}";
      compEndUpTo = n: if n == 1 then ":${compRegex}?" else "(:|${compEndRegex}{1,${toString n}})";
    in
  ...
  parseIp6 =
    ...
    let
      ...
      halves = map (x: if x == "" then [ ] else map parseHex (lib.splitString ":" x)) (
        lib.splitString "::" s
      );
  ...
  serializeIp6 =
    ...
    if maxStart == -1 then
      ...
      join (lib.take maxStart hextets) + "::" + join (lib.drop (maxStart + maxLen) hextets);
