...
let
  ...
  mkNftStartCmd =
    ...
    let
      ...
      injectStopRules =
        ...
        && (haveStopTextRules -> haveTextRules)
        && (haveStopJsonRules -> haveJsonRules);
      nft = "${pkgs.nftables}/bin/nft";
      fallback = d: x: if x != null then x else d;
    in
    ''
      ${lib.optionalString (!injectStopRules) (mkNftStopCmd attrs)}
      ${lib.optionalString (!jsonAfter && haveJsonRules)
        "${nft} -j ${nftFlags} -f ${
          pkgs.writeTextFile {
            name = "nftables-ruleset.json";
            text = builtins.toJSON {
              nftables =
                (lib.optionals injectStopRules (
                  attrs.nftables.stopJsonRules.nftables or [ { flush.ruleset = null; } ]
                ))
      ...
      ${lib.optionalString haveTextRules "${nft} ${nftFlags} -f ${
        pkgs.writeTextFile {
          name = "nftables-ruleset.nft";
          text =
            (lib.optionalString injectStopRules (
              (fallback "flush ruleset" attrs.nftables.stopTextRules) + "\n"
            ))
      ...
      ${lib.optionalString (
        haveTextFile && !injectTextFile
      ) "${nft} ${nftFlags} -f ${attrs.nftables.textFile}"}
  ...
  mkNftStopCmd =
    ...
    ''
      ...
      ${lib.optionalString haveStopTextRules "${pkgs.nftables}/bin/nft ${nftFlags} -f ${
        pkgs.writeTextFile {
          name = "nftables-ruleset.nft";
      ...
      ${lib.optionalString haveStopJsonRules "${pkgs.nftables}/bin/nft -j ${nftFlags} -f ${
        pkgs.writeTextFile {
          name = "nftables-ruleset.json";
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf (cfg.enable && config.networking.nftables.enable) {
      router.networkNamespaces.default =
        ...
        in
        lib.mkIf (rulesetFile != null || ruleset != "" || tables != { }) {
    ...
    (lib.mkIf (cfg.enable && enableNftables) {
      ...
      systemd.services =
        router-lib.zipHeads (
          lib.flip lib.mapAttrsToList (lib.filterAttrs (_: hasNftablesRules) cfg.networkNamespaces) (
            name: value: {
              "nftables-netns-${name}" = {
