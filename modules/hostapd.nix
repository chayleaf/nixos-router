        ...
        systemd.services = lib.flip lib.mapAttrs' cfg.interfaces (
          ...
          let
            ...
            compileValue =
              ...
              if builtins.isBool v then
                ...
                builtins.concatStringsSep " " (map (compileValue k) v)
              else if k == "ssid2" then
                "P${builtins.toJSON (toString v)}"
              else
                toString v;
            compileSettings =
              x:
              ...
              if !x ? ssid && !x ? ssid2 then
                ...
                x.wpa_key_mgmt == defaultSettings.wpa_key_mgmt && !x ? wpa_passphrase && !x ? sae_password
              then
          ...
          in
          {
            ...
            value = lib.mkIf icfg.hostapd.enable (
              router-lib.mkServiceForIf interface rec {
