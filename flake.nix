{
  ...
  outputs =
    ...
    let
      forEachSystem =
        func:
        nixpkgs.lib.genAttrs [ "aarch64-linux" "aarch64-darwin" "x86_64-darwin" "x86_64-linux" ] (
          system:
          func {
            inherit system;
              ...
              // {
                declarations = map (
                  ...
                  if lib.hasPrefix (toString ./.) (toString decl) then
                    ...
                    in
                    {
                      url = "https://github.com/chayleaf/nixos-router/blob/${self.sourceInfo.rev or "master"}/${subpath}";
                      name = subpath;
                    }
                  else
                    decl
