...
let
  ...
  inherit (router-lib) parseIp serializeIp invMask ip4Regex ip6Regex;
  backtripIp = a: b:
    assert matches (if lib.hasInfix ":" a then ip6Regex else ip4Regex) a;
    backtrip parseIp serializeIp a b;
in

assert backtripIp "0.0.0.0" [ 0 0 0 0 ];
assert backtripIp "127.0.0.1" [ 127 0 0 1 ];
assert backtripIp "255.255.255.255" [ 255 255 255 255 ];
assert backtripIp "::" [ 0 0 0 0 0 0 0 0 ];
assert backtripIp "a::" [ 10 0 0 0 0 0 0 0 ];
assert backtripIp "::a" [ 0 0 0 0 0 0 0 10 ];
assert backtripIp "a:a:a:a:a:a:a:a" [ 10 10 10 10 10 10 10 10 ];
assert backtripIp "ffff::ffff" [ 65535 0 0 0 0 0 0 65535 ];
assert backtripIp "ffff:ffff:ffff::ffff" [ 65535 65535 65535 0 0 0 0 65535 ];
assert backtripIp "ffff:ffff:ffff:ffff:ffff:ffff:ffff::" [ 65535 65535 65535 65535 65535 65535 65535 0 ];
assert backtripIp "ffff::ffff:ffff:ffff" [ 65535 0 0 0 0 65535 65535 65535 ];
assert eq (invMask [ 255 128 0 0 ]) [ 0 127 255 255 ];
assert eq (invMask [ 65535 32768 0 0 0 0 0 0 ]) [ 0 32767 65535 65535 65535 65535 65535 65535 ];
